    ...
    function(
        ...
        return Component.extend({
            ...
            updateOrder: function() {
                return (window.checkoutConfig.payment.micuentaweb_standard.updateOrder
                    && (window.checkoutConfig.payment.micuentaweb_standard.updateOrder == '1'))
                    || false;
            },
            ...
            manageError: function (e) {
                ...
                if (! PLACE_ORDER) {
                    storage.post(
                        url.build('micuentaweb/payment_rest/token?micuentaweb_action=restore_cart&form_key=' + $.mage.cookies.get('form_key'))
            ...
            refreshToken: function() {
                ...
                storage.post(
                    url.build('micuentaweb/payment_rest/token?micuentaweb_action=refresh_token&form_key=' + $.mage.cookies.get('form_key'))
                ).done(function(response) {
                    if (response.token) {
                        KR.setFormConfig({
            ...
            setToken: function() {
                ...
                storage.post(
                    url.build('micuentaweb/payment_rest/token?micuentaweb_action=set_token&form_key=' + $.mage.cookies.get('form_key'))
                ).done(function(response) {
                    if (response.token) {
                        KR.setFormConfig({
                            formToken: response.token,
                            ...
                            function(v) {
                                ...
                                KR.onError(function(e) {
                                    ...
                                    if (answer && (answer.clientAnswer.orderStatus == "UNPAID") && (answer.clientAnswer.orderCycle == "CLOSED")) {
                                        ...
                                        data['micuentaweb_update_order'] = true;
